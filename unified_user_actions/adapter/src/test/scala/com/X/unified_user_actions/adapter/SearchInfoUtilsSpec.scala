...
import com.X.suggests.controller_data.search_response.user_types.thriftscala.UserTypesControllerData
...
import com.X.unified_user_actions.adapter.client_event.SearchInfoUtils
...
class SearchInfoUtilsSpec
  ...
  test("getQueryOptFromControllerDataFromItem should return query if present in controller data") {
    new Fixture {
      ...
      val controllerData: ControllerData = mkControllerData(Some("X"))
  ...
  test("getQueryOptFromSearchDetails should return query if present") {
    new Fixture {
      ...
      val searchDetails: SearchDetails = SearchDetails(query = Some("X"))
  ...
  test("getQuerySourceOptFromControllerDataFromItem should return QuerySource if present") {
    new Fixture {
      ...
      // 1 is Typed Query
      val controllerData: ControllerData = mkControllerData(Some("X"), Some(1))
  ...
  test("getQuerySourceOptFromControllerDataFromItem should return None if not present") {
    new Fixture {
      ...
      val controllerData: ControllerData = mkControllerData(Some("X"), None)
