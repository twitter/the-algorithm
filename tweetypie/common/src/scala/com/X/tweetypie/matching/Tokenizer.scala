package com.X.tweetypie.matching

import com.X.common.text.language.LocaleUtil
import com.X.common_internal.text.pipeline.XTextNormalizer
import com.X.common_internal.text.pipeline.XTextTokenizer
import com.X.common_internal.text.version.PenguinVersion
import com.X.concurrent.Once
import com.X.io.StreamIO
...
object Tokenizer {
  ...
  /**
   * Encapsulates the configuration and use of [[XTextTokenizer]]
   * and [[XTextNormalizer]].
   */
  private[this] class TokenizerFactory(version: PenguinVersion) {
    ...
    private[this] val normalizer =
      (new XTextNormalizer.Builder(version)).build()

    // The XTextTokenizer is relatively expensive to build,
    // and is not thread safe, so keep instances of it in a
    // ThreadLocal.
    private[this] val local =
      new ThreadLocal[XTextTokenizer] {
        override def initialValue: XTextTokenizer =
          (new XTextTokenizer.Builder(version)).build()
      }
    ...
    def forLocale(locale: Locale): Tokenizer =
      new Tokenizer {
        override def tokenize(input: String): TokenSequence = {
          val stream = local.get.getXTokenStreamFor(locale)
   ...
   * the logic of XTextTokenizer and XTextNormalizer.
   */
  ...
  /**
   * The creation of the first XTextTokenizer is relatively
