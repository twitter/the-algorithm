...
import com.X.servo.data.Mutation
import com.X.tco_util.TcoUrl
...
object EntityExtractor {
  // We only use one configuration of com.X.Xtext.Extractor, so it's
  ...
  // The X-text library operates on unencoded text, but we store
  ...
  // the X-text entities back to the entities on the encoded text.
