package com.X.tweetypie.context
...
import com.X.io.Buf
import com.X.io.Buf.ByteArray.Owned
import com.X.finagle.stats.StatsReceiver
import com.X.graphql.common.core.GraphQlClientApplication
...
object TweetypieContext {
  ...
  object Broadcast {
    ...
    // ctx has to be by name so we can re-evaluate it for every request (for usage in ServiceX.scala)
  ...
  object Local {
    ...
    def trackStats[U](scopes: StatsReceiver*): Unit =
      for {
        ...
        XCtx <- XContext()
        clientAppId <- XCtx.clientApplicationId
      } yield {
