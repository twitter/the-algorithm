package com.X.servo.cache

import com.X.finagle.memcached.Client
import com.X.finagle.memcached.protocol.Value
import com.X.finagle.memcached.GetResult
import com.X.finagle.memcached.ProxyClient
...
/**
 * Wrapper for a [[com.X.finagle.Memcached.Client]] that handles in-process caching for
 * values flagged for promotion ("hot keys") by a twemcache backend.
 *
 * This is similar conceptually to
 * [[com.X.servo.repository.HotKeyCachingKeyValueRepository]] but differs because
 ...
 *  accessed keys. This is a custom feature in twemcache, X's memcache fork, that is not
...
// TOOD: May want to turn flags into a value class in com.X.finagle.memcached
