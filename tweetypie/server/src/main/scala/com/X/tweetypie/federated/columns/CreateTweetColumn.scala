...
import com.X.geo.model.LatitudeLongitude
...
import com.X.tweetypie.decider.overrides.TweetyPieDeciderOverrides
import com.X.tweetypie.federated.columns.ApiErrors._
import com.X.tweetypie.federated.columns.CreateTweetColumn.toCreateTweetErr
...
import com.X.tweetypie.federated.promotedcontent.TweetPromotedContentLogger._
import com.X.tweetypie.repository.UnmentionInfoRepository
import com.X.tweetypie.repository.VibeRepository
...
import com.X.tweetypie.util.ConversationControls
import com.X.tweetypie.util.TransientContextUtil
...
class CreateTweetColumn(
  ...
  override val metadata: OpMetadata =
    OpMetadata(
      ...
      Some(
        PlainText(
          """
    Creates a tweet using the calling authenticated X user as author. 
  ...
  private[this] def parseTransientContext(
    ...
    val periscopeCreatorId = if (periscopeIsLive.isDefined) Some(XUserId) else None
