...
import com.X.tweetypie.repository.ParentUserIdRepository
import com.X.tweetypie.storage.TweetStorageClient.Undelete
...
import com.X.tweetypie.store.UndeleteTweet
import com.X.tweetypie.thriftscala.UndeleteTweetState.{Success => TweetypieSuccess, _}
 ...
 * problem with the underlying data in Manhattan or a bug in [[com.X.tweetypie.storage.TweetStorageClient]]
 */
    ...
    def getValidatedDeletedTweet(
      ...
      val deletedTweet = getDeletedTweet(tweetId).map { response =>
        response.deleteState match {
          ...
          // BounceDeleted tweets violated X Rules and may not be undeleted
