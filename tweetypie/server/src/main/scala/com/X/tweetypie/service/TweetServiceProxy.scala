...
import com.X.tweetypie.thriftscala.{TweetServiceProxy => BaseTweetServiceProxy, _}

/**
 * A trait for TweetService implementations that wrap an underlying TweetService and need to modify
 * only some of the methods.
 *
 * This proxy is the same as [[com.X.tweetypie.thriftscala.TweetServiceProxy]], except it also
 * extends [[com.X.tweetypie.thriftscala.TweetServiceInternal]] which gives us access to all
