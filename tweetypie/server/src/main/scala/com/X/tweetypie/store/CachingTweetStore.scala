...
import com.X.servo.cache.LockingCache._
import com.X.servo.cache._
import com.X.tweetypie.additionalfields.AdditionalFields
import com.X.tweetypie.repository.CachedBounceDeleted.isBounceDeleted
import com.X.tweetypie.repository.CachedBounceDeleted.toBounceDeletedCachedTweet
import com.X.tweetypie.repository._
import com.X.tweetypie.store.TweetUpdate._
...
private class CachingTweetStoreOps(
  ...
  private val cacheFlushesLog = Logger("com.X.tweetypie.store.CacheFlushesLog")
  ...
  def deleteTweet(tweetId: TweetId, updateOnly: Boolean, isBounceDelete: Boolean): Future[Unit] = {
    ...
    // timeline tombstones for tweets that violated the X Rules. see go/bounced-tweet
