...
import com.X.gizmoduck.thriftscala.AccessPolicy
...
import com.X.tweetypie.additionalfields.AdditionalFields._
import com.X.tweetypie.core._
import com.X.tweetypie.jiminy.tweetypie.NudgeBuilder
import com.X.tweetypie.jiminy.tweetypie.NudgeBuilderRequest
import com.X.tweetypie.media.Media
import com.X.tweetypie.repository.StratoCommunityAccessRepository.CommunityAccess
...
import com.X.tweetypie.tweettext._
import com.X.tweetypie.util.CommunityAnnotation
import com.X.tweetypie.util.CommunityUtil
import com.X.Xtext.Regex.{VALID_URL => UrlPattern}
import com.X.Xtext.XTextParser
...
object TweetBuilder {
  ...
  /**
   * Checks the weighted tweet text length using X-text, as used by clients.
  ...
  def prevalidateTextLength(text: String, stats: StatsReceiver): Future[Unit] = {
    val XTextConfig = XTextParser.TWITTER_TEXT_DEFAULT_CONFIG
    val XTextResult = XTextParser.parseTweet(text, XTextConfig)
    val textTooLong = !XTextResult.isValid && text.length > 0

    Future.when(textTooLong) {
      val weightedLength = XTextResult.weightedLength
      log.debug(
  ...
  def apply(
    ...
    val permalinkShortener = (tweetId: TweetId, ctx: UrlShortener.Context) =>
      urlShortener((s"https://X.com/i/web/status/$tweetId", ctx)).rescue {
