...
class DefaultTweetDeletePathHandler(
  ...
  private[this] val log = Logger("com.X.tweetypie.store.TweetDeletions")
  ...
  /**
   * Fill in missing values of AuditDeleteTweet with values from XContext.
   */
  def enrichMissingFromXContext(orig: AuditDeleteTweet): AuditDeleteTweet = {
    val viewer = XContext()
    orig.copy(
  ...
  override def internalDeleteTweets(
    ...
    val auditDeleteTweet =
      enrichMissingFromXContext(request.auditPassthrough.getOrElse(AuditDeleteTweet()))
