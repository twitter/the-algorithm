...
import com.X.servo.util.Scribe
import com.X.tweetypie.backends.LimiterService.Feature.MediaTagCreate
import com.X.tweetypie.backends.LimiterService.Feature.Updates
import com.X.tweetypie.client_id.ClientIdHelper
import com.X.tweetypie.handler.TweetBuilder
import com.X.tweetypie.repository.TweetKeyFactory
import com.X.tweetypie.store._
import com.X.tweetypie.tflock.TFlockIndexer
...
object TweetStores {
  def apply(
    ...
    val logLensStore =
      LogLensStore(
        tweetCreationsLogger = Logger("com.X.tweetypie.store.TweetCreations"),
        tweetDeletionsLogger = Logger("com.X.tweetypie.store.TweetDeletions"),
        tweetUndeletionsLogger = Logger("com.X.tweetypie.store.TweetUndeletions"),
        tweetUpdatesLogger = Logger("com.X.tweetypie.store.TweetUpdates"),
        clientIdHelper = clientIdHelper,
      )
