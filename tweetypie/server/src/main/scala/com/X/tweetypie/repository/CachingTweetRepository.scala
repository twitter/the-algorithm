...
import com.X.tweetypie.repository.CachedBounceDeleted.toBounceDeletedTweetResult
...
object CachingTweetRepository {
  ...
  val failuresLog: Logger = Logger("com.X.tweetypie.repository.CachingTweetRepoFailures")
    ...
    case ((tweetId, options), tweetResult) =>
      if (!options.cacheControl.writeToCache) {
        ...
        tweetResult match {
          ...
          case Throw(com.X.stitch.NotFound) =>
