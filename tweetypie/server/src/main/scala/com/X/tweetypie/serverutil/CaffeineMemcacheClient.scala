...
import com.X.util.{Promise => XPromise}
...
object CaffeineMemcacheClient {
  ...
  /**
   * Helper method to convert between Java 8's CompletableFuture and X's Future.
   */
  private def toXFuture[T](cf: CompletableFuture[T]): Future[T] = {
    if (cf.isDone && !cf.isCompletedExceptionally && !cf.isCancelled) {
      Future.const(Return(cf.get()))
    } else {
      val p = new XPromise[T] with XPromise.InterruptHandler {
...
class CaffeineMemcacheClient(
  ...
  override def getResult(keys: Iterable[String]): Future[GetResult] = {
    val XFuture = toXFuture(cache.getAll(keys.asJava))
    XFuture
      .map { result =>
