...
import com.X.botmaker.thriftscala.BotMakerResponse
import com.X.bouncer.thriftscala.Bounce
import com.X.finagle.tracing.Trace
import com.X.relevance.feature_store.thriftscala.FeatureData
import com.X.relevance.feature_store.thriftscala.FeatureValue.StrValue
...
object Spam {
  ...
  def DisabledByIpiFailure(
    ...
    val errorMsg = (customDenyMessage, userName) match {
      ...
      case (_, Some(name)) => s"Some actions on this ${name} Tweet have been disabled by X."
      case _ => "Some actions on this Tweet have been disabled by X."
    }
  ...
  def handleScarecrowResult(
    ...
    result => {
      ...
      Trace.record("com.X.tweetypie.Spam.scarecrow_result=" + result.resultCode.name)
      /*
