...
import com.X.tweetypie.backends.LimiterService.Feature
import com.X.tweetypie.handler._
import com.X.tweetypie.jiminy.tweetypie.NudgeBuilder
import com.X.tweetypie.repository.RelationshipKey
import com.X.tweetypie.store.TotalTweetStore
...
object TweetBuilders {
  ...
  def validateCardRefAttachmentByUserAgentGate(
    ...
    Gate[Option[String]] { (userAgent: Option[String]) =>
      if (userAgent.exists(_.startsWith("XAndroid"))) {
        android()
      } else {
        nonAndroid()
      }
  ...
  def apply(
    ...
    val editControlBuilder = EditControlBuilder(
      ...
      checkXBlueSubscription = deciderGates.checkXBlueSubscriptionForEdit,
