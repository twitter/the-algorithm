...
import com.X.gizmoduck.util.UserUtil
import com.X.stitch.Stitch
import com.X.tweetypie.core.TweetCreateFailure
import com.X.tweetypie.repository.Card2Repository
import com.X.tweetypie.repository.StratoPromotedTweetRepository
import com.X.tweetypie.repository.StratoSubscriptionVerificationRepository
import com.X.tweetypie.repository.TweetQuery
import com.X.tweetypie.repository.TweetRepository
import com.X.tweetypie.repository.UrlCard2Key
...
import com.X.tweetypie.util.EditControlUtil._
import com.X.tweetypie.thriftscala.CardReference
import com.X.tweetypie.thriftscala.EditControlInitial
import com.X.tweetypie.thriftscala.PostTweetRequest
import com.X.tweetypie.util.CommunityAnnotation
...
object EditControlBuilder {
  ...
  val TweetEditCreationEnabledForXBlueKey = "tweet_edit_creation_enabled_for_X_blue"
  ...
  def apply(
    ...
    checkXBlueSubscription: Gate[Unit],
    ...
    // - TweetEditCreationEnabledForXBlueKey authorizes the user to edit tweets if they have
    //     a X Blue subscription
    //
    ...
    def checkUserEligibility(
      ...
      val authorizedWithoutXBlue =
        isFeatureSwitchEnabled(matchedResults, TweetEditCreationEnabledKey)

      if (isTestUser || authorizedWithoutXBlue) {
        // If the editing user is a test user or is authorized by the non-X Blue feature
        // switch, allow editing.
        Stitch.Done
      } else {
        // Otherwise, check if they're authorized by the X Blue feature switch and if they're
        // subscribed to X Blue.
        val authorizedWithXBlue: Stitch[Boolean] =
          if (checkXBlueSubscription() &&
            isFeatureSwitchEnabled(matchedResults, TweetEditCreationEnabledForXBlueKey)) {
            subscriptionVerificationRepo(authorId, tweetEditSubscriptionResource)
          } else Stitch.value(false)

        authorizedWithXBlue.flatMap { authorized =>
          if (!authorized) {
