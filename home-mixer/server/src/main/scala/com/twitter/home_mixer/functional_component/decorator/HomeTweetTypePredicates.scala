  ...
  private[this] val CandidatePredicates: Seq[(String, FeatureMap => Boolean)] = Seq(
    ...
    (
      ...
      candidate =>
        candidate
          .getOrElse(AuthorIdFeature, None)
          .exists(candidate.getOrElse(DDGStatsGreenPartyFeature, Set.empty[Long]).contains)),
      "author_is_independent",
      candidate =>
        candidate
          .getOrElse(AuthorIdFeature, None)
          .exists(candidate.getOrElse(DDGStatsIndependentFeature, Set.empty[Long]).contains)),
      "author_is_socialist",
      candidate =>
        candidate
          .getOrElse(AuthorIdFeature, None)
          .exists(candidate.getOrElse(DDGStatsSocialistFeature, Set.empty[Long]).contains)),
  )
