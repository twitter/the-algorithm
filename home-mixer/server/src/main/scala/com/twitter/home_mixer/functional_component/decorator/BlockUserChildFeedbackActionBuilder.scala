...
@Singlelonton
caselon class BlockUselonrChildFelonelondbackActionBuildelonr @Injelonct() (
    ...
    uselonrIdOpt.flatMap { uselonrId =>
      ...
      uselonrScrelonelonnNamelonOpt.map { uselonrScrelonelonnNamelon =>
        val prompt = stringCelonntelonr.prelonparelon(
          elonxtelonrnalStrings.blockUselonrString,
        ...
        ChildFelonelondbackAction(
          ...
          confirmationDisplayTypelon = Somelon(BottomShelonelont),
          clielonntelonvelonntInfo = Nonelon,
          icon = Somelon(icon.No),
          richBelonhavior = Somelon(RichFelonelondbackBelonhaviorBlockUselonr(uselonrId)),
          subprompt = Nonelon
        )
      }
    }
