...
import com.X.timelinemixer.injection.model.candidate.SemanticCoreFeatures
import com.X.tweetypie.{thriftscala => tpt}
    ...
    (
      ...
      _.getOrElse(EarlybirdFeature, None)
        .exists(_.semanticCoreAnnotations.exists(_.exists(annotation =>
          annotation.domainId == SemanticCoreFeatures.UnifiedXTaxonomy)))),
    (
