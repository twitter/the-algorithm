...
import com.X.frigate.common.util.{SnowflakeUtils, UrlInfo}
import com.X.storehaus.{FutureOps, ReadableStore}
import com.X.util.{Duration, Future, Timer}
...
class UrlResolver(
  ...
  private val XResolvedUrlCounter = stats.counter("XResolvedUrl")
          ...
          case (
                ...
                Some(
                  UrlInfo(
                    ...
                    _,
                    _))) =>
            if (domain == "X") { // Filter out X based URLs
              XResolvedUrlCounter.incr()
              None
            } else {
