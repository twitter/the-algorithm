package com.X.cr_mixer.service

import com.X.product_mixer.core.functional_component.common.alert.Destination
import com.X.product_mixer.core.functional_component.common.alert.NotificationGroup
 ...
 * https://docbird.X.biz/mon/how-to-guides.html?highlight=notificationgroup#set-up-email-pagerduty-and-slack-notifications
 */
object CrMixerAlertNotificationConfig {
  val DefaultNotificationGroup: NotificationGroup = NotificationGroup(
    warn = Destination(emails = Seq("no-reply@X.com")),
    critical = Destination(emails = Seq("no-reply@X.com"))
  )
}
