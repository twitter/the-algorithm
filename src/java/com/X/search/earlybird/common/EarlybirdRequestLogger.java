...
import com.X.search.common.logging.RPCLogger;
import com.X.search.common.metrics.FailureRatioCounter;
import com.X.search.common.metrics.Timer;
import com.X.search.common.util.earlybird.TermStatisticsUtil;
import com.X.search.common.util.earlybird.ThriftSearchResultUtil;
...
import com.X.search.earlybird.thrift.ThriftHistogramSettings;
...
public class EarlybirdRequestLogger extends RPCLogger {
  ...
  private void setRequestLogEntries(LogEntry entry, EarlybirdRequest request) {
    ...
    setLogEntriesFromXContext(entry);
  }

  @VisibleForTesting
  Option<Viewer> getXContext() {
    return XContext.acquire(XContextPermit.get()).apply();
  }

  private void setLogEntriesFromXContext(LogEntry entry) {
    Option<Viewer> viewerOption = getXContext();
