...
public final class CachedFilterQuery extends Query {
  ...
  public Query rewrite(IndexReader reader) {
    EarlybirdIndexSegmentAtomicReader XReader = (EarlybirdIndexSegmentAtomicReader) reader;
    QueryCacheResultForSegment cachedResult =
        XReader.getSegmentData().getQueryCacheResult(queryCacheFilter.getFilterName());
    ...
    if (!queryCacheFilter.getCacheModeOnly() && // is this a cache mode only filter?
        ...
        cachedResult.getSmallestDocID() > XReader.getSmallestDocID()) {
