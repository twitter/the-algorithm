package com.X.search.earlybird.search;
...
import com.X.search.core.earlybird.index.TimeMapper;
import com.X.search.core.earlybird.index.inverted.QueryCostTracker;
...
import com.X.search.earlybird.index.TweetIDMapper;
import com.X.search.earlybird.search.facets.FacetLabelCollector;
import com.X.search.earlybird.stats.EarlybirdSearcherStats;
import com.X.search.earlybird.thrift.ThriftFacetLabel;
...
import com.X.search.earlybird.thrift.ThriftSearchResultGeoLocation;
...
public abstract class AbstractResultsCollector<R extends SearchRequestInfo,
  ...
  protected EarlybirdIndexSegmentAtomicReader currXReader;
  ...
  private void updateFirstSearchedStatusID() {
    ...
    // See SEARCH-4535
    int minDocID = currXReader.getSmallestDocID();
    if (currXReader.hasDocs() && minDocID >= 0 && !searchedStatusIDsAndTimesInitialized()) {
  ...
  public final void setNextReader(LeafReader reader) throws IOException {
    ...
    currXReader = (EarlybirdIndexSegmentAtomicReader) reader;
    documentFeatures = new EarlybirdDocumentFeatures(currXReader);
    tweetIdMapper = (TweetIDMapper) currXReader.getSegmentData().getDocIDToTweetIDMapper();
    timeMapper = currXReader.getSegmentData().getTimeMapper();
    currTimeSliceID = currXReader.getSegmentData().getTimeSliceID();
  ...
  protected void collectFacets(ThriftSearchResultMetadata metadata) {
    ...
    AbstractFacetCountingArray facetCountingArray = currXReader.getFacetCountingArray();
    EarlybirdIndexSegmentData segmentData = currXReader.getSegmentData();
  ...
  public void skipSegment(EarlybirdSingleSegmentSearcher searcher) throws IOException {
    setNextReader(searcher.getXIndexReader().getContext());
  ...
  protected final void fillResultGeoLocation(ThriftSearchResultMetadata metadata)
    ...
    if (currXReader != null && fillInLatLonForHits) {
