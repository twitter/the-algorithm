...
import com.X.search.common.util.spatial.GeoUtil;
...
import com.X.search.core.earlybird.index.inverted.MPHTermDictionary;
import com.X.search.core.earlybird.index.inverted.RealtimeIndexTerms;
import com.X.search.earlybird.index.EarlybirdSingleSegmentSearcher;
...
public class IndexViewer {
   ...
   * {@link com.X.search.common.util.analysis.IntTermAttribute}
   */
  ...
  /**
   * Fields whose terms are indexed using
   * {@link com.X.search.common.util.analysis.LongTermAttribute}
   */
  ...
  private static class TermDto {
    ...
    public void write(ViewerWriter writer,
                      EarlybirdIndexSegmentAtomicReader XReader) throws IOException {
      writer.beginObject();
      ...
      if (docsEnum != null) {
        appendFrequencyAndPositions(writer, field, docsEnum, XReader);
      }
      if (maxDocs != null) {
        appendDocs(writer, termsEnum, maxDocs, XReader);
      }
      writer.endObject();
    }
  }
  ...
  public IndexViewer(EarlybirdSingleSegmentSearcher searcher) {
    ...
    this.XReader = searcher.getXIndexReader();
  }
  ...
  public void dumpTweetDataByTweetId(ViewerWriter writer, long tweetId, Options options)
      throws IOException {
    int docId = XReader.getSegmentData().getDocIDToTweetIDMapper().getDocID(tweetId);
  ...
  public void dumpTweetDataByDocId(ViewerWriter writer, int docId, Options options)
    ...
    printHeader(writer);
    long tweetID = XReader.getSegmentData().getDocIDToTweetIDMapper().getTweetID(docId);
    if (docId < XReader.maxDoc() && tweetID >= 0) {
  ...
  private void writeTweetIdsToLogFile(PrintWriter logWriter) {
    DocIDToTweetIDMapper mapper = XReader.getSegmentData().getDocIDToTweetIDMapper();
    ...
    while ((docId = mapper.getNextDocID(docId)) != DocIDToTweetIDMapper.ID_NOT_FOUND) {
      ...
      if ((tweetId > 0) && !XReader.getDeletesView().isDeleted(docId)) {
  ...
  private void dumpCSFData(ViewerWriter writer, int docId) throws IOException {
    Schema tweetSchema = XReader.getSchema();
    ...
    for (Schema.FieldInfo fieldInfo: sortedFieldInfos) {
      ...
      NumericDocValues csfDocValues = XReader.getNumericDocValues(csfFieldInfoName);
      // If XReader.getNumericDocValues(value.getName()) == null,
  ...
  private void dumpTweetData(ViewerWriter writer,
    ...
    Terms terms = XReader.terms(field);
    if (terms != null) {
  ...
  private List<String> sortedFields() {
    ...
    for (Schema.FieldInfo fieldInfo : XReader.getSchema().getFieldInfos()) {
  ...
  private void printTerm(ViewerWriter writer, String field, TermsEnum termsEnum,
    ...
    double dfPercent = ((double) df / this.XReader.numDocs()) * 100.0;
  ...
  private static void appendDocs(ViewerWriter writer, TermsEnum termsEnum, int maxDocs,
                                 EarlybirdIndexSegmentAtomicReader XReader)
      throws IOException {
    writer.name("docIds");
  ...
  private void printHeader(ViewerWriter writer) throws IOException {
    ...
    writer.name("maxDocNumber").value(Integer.toString(this.XReader.maxDoc()));
    writer.newline();
  }
  ...
  public void dumpSchema(ViewerWriter writer) throws IOException {
    ...
    writer.beginArray();
    writer.newline();
    Schema schema = this.XReader.getSchema();
