...
/**
 * An IndexSearch that works with XEarlyTerminationCollector.
 * If a stock Lucene collector is passed into search(), this IndexSearch.search() behaves the
 * same as Lucene's stock IndexSearcher.  However, if a XEarlyTerminationCollector is passed
...
public class XIndexSearcher extends IndexSearcher {
  ...
  protected void search(List<LeafReaderContext> leaves, Weight weight, Collector coll)
    ...
    if (coll instanceof XCollector) {
      ...
      // The collector given is not a XCollector, just use stock lucene search().
