...
import com.X.common.text.transformer.HTMLTagRemovalTransformer;
import com.X.common_internal.text.extractor.EmojiExtractor;
...
public final class XMessageUtil {
  private static final Logger LOG = LoggerFactory.getLogger(XMessageUtil.class);

  private XMessageUtil() {
  }
  ...
  private static final String STAT_PREFIX = "X_message_";
  ...
  /**
   * Sets the origSource and strippedSource fields on a XMessage
   *
   */
  public static void setSourceOnMessage(XMessage message, String modifiedDeviceSource) {
    // Always strip emojis from sources: they don't really make sense in this field.
   ...
   */
  public static boolean validateXMessage(
      XMessage message,
  ...
  private static boolean validateRetweetMessage(
      XRetweetMessage message, Set<Field> stripEmojisForFields) {
    if (message.getSharedId() == null || message.getRetweetId() == null) {
      LOG.error("Retweet Message contains a null X id. Skipping.");
