...
import com.X.search.common.constants.SearchCardType;
import com.X.search.common.decider.DeciderUtil;
import com.X.search.common.indexing.thriftjava.SearchCard2;
...
import com.X.search.common.indexing.thriftjava.XPhotoUrl;
...
import com.X.search.common.schema.thriftjava.ThriftField;
import com.X.search.common.schema.thriftjava.ThriftFieldData;
 ...
 * contained in a XMessage. This data is delayed because these services take a few seconds
...
public class DelayedIndexingConverter {
   ...
   *
   * @param message The XMessage to convert.
  ...
  public List<ThriftVersionedEvents> convertMessageToOutOfOrderAppendAndFeatureUpdate(
      XMessage message, List<PenguinVersion> penguinVersions) {
    ...
    for (PenguinVersion penguinVersion : penguinVersions) {
      ...
      message.getFromUserXId().ifPresent(outOfOrderAppendThriftIndexingEvent::setUid);
  ...
  private ThriftDocumentWithEncodedTweetFeatures buildDocumentForPenguinVersion(
    ...
    EarlybirdEncodedFeatures encodedFeatures = featureBuilder.createTweetFeaturesFromXMessage(
  ...
  public static void buildNamedEntityFields(
      EarlybirdThriftDocumentBuilder builder, XMessage message) {
  ...
  private void buildAnalyzedURLFields(
      EarlybirdThriftDocumentBuilder builder, XMessage message, PenguinVersion penguinVersion
  ) {
    ...
    if (DeciderUtil.isAvailableForRandomRecipient(
      ...
      /*
        ...
        TweetEventParserHelper.addMediaEntitiesToMessage. These X.com media URLs contain
        the tweet text as the description and the title is "<User Name> on X". This is
  ...
  public static void buildURLFields(
    ...
    // now add all X photos links that came with the tweet's payload
    Map<Long, String> photos = message.getPhotoUrls();
    List<XPhotoUrl> photoURLs = new ArrayList<>();
    if (photos != null) {
      for (Map.Entry<Long, String> entry : photos.entrySet()) {
        XPhotoUrl photo = new XPhotoUrl(entry.getKey());
  ...
  static void buildCardFields(EarlybirdThriftDocumentBuilder builder,
                              ...
                              PenguinVersion penguinVersion) {
    if (message.hasCard()) {
      SearchCard2 card = buildSearchCardFromXMessage(
          message,
  ...
  private static SearchCard2 buildSearchCardFromXMessage(
      XMessage message,
    ...
    if (message.getCardTitle() != null && !message.getCardTitle().isEmpty()) {
      ...
      try {
        ...
        LOG.error("XTokenStream serialization error! Could not serialize card title: "
    ...
    if (message.getCardDescription() != null && !message.getCardDescription().isEmpty()) {
      ...
      try {
        ...
        LOG.error("XTokenStream serialization error! Could not serialize card description: "
            + result.tokenSequence);
  ...
  private void buildSpaceAdminFields(
      EarlybirdThriftDocumentBuilder builder,
      Set<XMessageUser> spaceAdmins,
      PenguinVersion penguinVersion) {

    for (XMessageUser spaceAdmin : spaceAdmins) {
  ...
  private void buildTweetTokenizerTokenizedField(
    ...
    if (StringUtils.isNotEmpty(text)) {
      ...
      try {
        ...
        LOG.error("XTokenStream serialization error! Could not serialize: " + text);
      }
    }
  }
}
