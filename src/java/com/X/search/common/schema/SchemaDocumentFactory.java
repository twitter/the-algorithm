...
import com.X.search.common.schema.base.IndexedNumericFieldSettings;
...
import com.X.search.common.schema.thriftjava.ThriftGeoCoordinate;
import com.X.search.common.util.analysis.IntTermAttribute;
import com.X.search.common.util.analysis.LongTermAttribute;
import com.X.search.common.util.analysis.SortableLongTermAttribute;
import com.X.search.common.util.spatial.GeoUtil;
import com.X.search.common.util.text.HighFrequencyTermPairs;
import com.X.search.common.util.text.OmitNormTextField;
import com.X.search.common.util.text.SingleTokenStream;

/**
 * A document factory that converts {@link ThriftDocument} into Lucene {@link Document}s
 * using the provided {@link com.X.search.common.schema.base.Schema}.
 */
public class SchemaDocumentFactory {
  ...
  private void addTokenField(
    ...
    try {
      for (TokenStreamRewriter rewriter : tokenStreamRewriters) {
        XTokenStream = rewriter.rewrite(fieldInfo, XTokenStream);
      }

      expandStream(doc, fieldInfo, XTokenStream, hfTerms, hfPhrases);
      doc.add(new Field(fieldInfo.getName(), XTokenStream, fieldInfo.getFieldType()));
    } finally {
      XTokenStream.close();
    }
  }
  ...
  private void addIntField(Document doc, Schema.FieldInfo fieldInfo,
    ...
    } else if (fieldInfo.getFieldType().getNumericFieldSettings().isUseXFormat()) {
  ...
  private void addLongField(Document doc, Schema.FieldInfo fieldInfo,
    ...
    if (fieldInfo.getFieldType().getNumericFieldSettings() == null) {
      ...
      // X style numerical field: use LongTermAttribute
  ...
  /**
   * Expand the XTokenStream and populate high-frequency terms, phrases and/or facet category paths.
   */
  ...
  public interface TokenStreamRewriter {
     ...
     */
    XTokenStream rewrite(Schema.FieldInfo fieldInfo, XTokenStream stream);
  }
}
