...
import com.X.cuad.ner.plain.thriftjava.NamedEntityContext;
import com.X.cuad.ner.plain.thriftjava.NamedEntityInputSourceType;
import com.X.cuad.ner.thriftjava.WholeEntityType;
...
import com.X.search.common.schema.ThriftDocumentBuilder;
...
import com.X.search.common.util.analysis.XPhotoTokenStream;
...
import com.X.search.common.util.url.URLUtils;
...
public final class EarlybirdThriftDocumentBuilder extends ThriftDocumentBuilder {
  ...
  /**
   * Add a list of X Photo URLs (twimg URLs). These are different from regular URLs, because
   * we use the XPhotoTokenStream to index them, and we also include the status ID as payload.
   */
  public EarlybirdThriftDocumentBuilder withTwimgURLs(
      List<XPhotoUrl> urls) throws IOException {
    if (isNotEmpty(urls)) {
      for (XPhotoUrl photoUrl : urls) {
        TokenStream ts = new XPhotoTokenStream(photoUrl.getPhotoStatusId(),
  ...
  public EarlybirdThriftDocumentBuilder withCamelCaseTokenizedScreenNameField(
    ...
    try {
      ...
      LOG.error("XTokenStream serialization error! Could not serialize: " + screenName);
