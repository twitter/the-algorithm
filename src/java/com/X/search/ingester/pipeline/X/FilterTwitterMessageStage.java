package com.X.search.ingester.pipeline.X;

import org.apache.commons.pipeline.StageException;
import org.apache.commons.pipeline.validation.ConsumedTypes;
import org.apache.commons.pipeline.validation.ProducesConsumed;

import com.X.search.common.metrics.SearchRateCounter;
import com.X.search.common.relevance.entities.XMessage;
import com.X.search.ingester.pipeline.X.filters.IngesterValidMessageFilter;
import com.X.search.ingester.pipeline.util.PipelineStageRuntimeException;

/**
 * Filter out X messages meeting some filtering rule.
 */
@ConsumedTypes(XMessage.class)
@ProducesConsumed
public class FilterXMessageStage extends XBaseStage
    <XMessage, XMessage> {
  private IngesterValidMessageFilter filter = null;
  private SearchRateCounter validMessages;
  private SearchRateCounter invalidMessages;

  @Override
  protected void initStats() {
    super.initStats();
    innerSetupStats();
  }

  @Override
  protected void innerSetupStats() {
    validMessages = SearchRateCounter.export(getStageNamePrefix() + "_valid_messages");
    invalidMessages = SearchRateCounter.export(getStageNamePrefix() + "_filtered_messages");
  }

  @Override
  protected void doInnerPreprocess() {
    innerSetup();
  }

  @Override
  protected void innerSetup() {
    filter = new IngesterValidMessageFilter(decider);
  }

  @Override
  public void innerProcess(Object obj) throws StageException {
    if (!(obj instanceof XMessage)) {
      throw new StageException(this, "Object is not a IngesterXMessage: "
      + obj);
    }

    XMessage message = (XMessage) obj;
    if (tryToFilter(message)) {
      emitAndCount(message);
    }
  }

  @Override
  protected XMessage innerRunStageV2(XMessage message) {
    if (!tryToFilter(message)) {
      throw new PipelineStageRuntimeException("Failed to filter, does not have to "
      + "pass to the next stage");
    }
    return message;
  }

  private boolean tryToFilter(XMessage message) {
    boolean ableToFilter = false;
    if (message != null && filter.accepts(message)) {
      validMessages.increment();
      ableToFilter = true;
    } else {
      invalidMessages.increment();
    }
    return ableToFilter;
  }
}
