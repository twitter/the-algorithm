...
import com.X.search.ingester.pipeline.X.thriftparse.ThriftTweetParsingException;
import com.X.search.ingester.pipeline.X.thriftparse.TweetEventParseHelper;
import com.X.tweetypie.thriftjava.TweetCreateEvent;
import com.X.tweetypie.thriftjava.TweetDeleteEvent;
import com.X.tweetypie.thriftjava.TweetEventData;

@ConsumedTypes(IngesterTweetEvent.class)
@ProducedTypes(IngesterXMessage.class)
public class ThriftTweetParserStage extends XBaseStage<IngesterTweetEvent, XMessage> {
  ...
  public void innerProcess(Object obj) throws StageException {
    ...
    try {
      ...
      IngesterXMessage message = getXMessage(tweetEventData, debugEvents);
      ...
      if (shouldEmitMessage) {
        if (!message.isDeleted()) {
          ...
          for (String tweetCreateEventBranch : tweetCreateEventBranches) {
            ...
            IngesterXMessage tweetCreateEventBranchMessage =
                getXMessage(tweetEventData, debugEvents);
          ...
          for (String tweetDeleteEventBranch : tweetDeleteEventBranches) {
            // If we need to send the message to another branch, we need to make a copy.
            // Otherwise, we'll have multiple stages mutating the same object in parallel.
            IngesterXMessage tweetDeleteEventBranchMessage =
                getXMessage(tweetEventData, debugEvents);
  ...
  @Nullable
  private IngesterXMessage getXMessage(
    ...
    if (tweetEventDataField == TweetEventData._Fields.TWEET_CREATE_EVENT) {
      ...
      return TweetEventParseHelper.getXMessageFromCreationEvent(
    ...
    if (tweetEventDataField == TweetEventData._Fields.TWEET_DELETE_EVENT) {
      ...
      return TweetEventParseHelper.getXMessageFromDeletionEvent(
