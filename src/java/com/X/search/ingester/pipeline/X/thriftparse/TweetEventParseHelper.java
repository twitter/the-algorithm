package com.X.search.ingester.pipeline.X.thriftparse;
...
import com.X.dataproducts.enrichments.thriftjava.GeoEntity;
import com.X.dataproducts.enrichments.thriftjava.PotentialLocation;
import com.X.dataproducts.enrichments.thriftjava.ProfileGeoEnrichment;
import com.X.escherbird.thriftjava.TweetEntityAnnotation;
import com.X.expandodo.thriftjava.Card2;
import com.X.gizmoduck.thriftjava.User;
import com.X.mediaservices.commons.tweetmedia.thrift_java.MediaInfo;
...
import com.X.search.common.relevance.entities.PotentialLocationObject;
import com.X.search.common.relevance.entities.XMessage;
import com.X.search.common.relevance.entities.XMessage.EscherbirdAnnotation;
import com.X.search.common.relevance.entities.XMessageUser;
import com.X.search.common.relevance.entities.XMessageUtil;
import com.X.search.common.relevance.entities.XQuotedMessage;
import com.X.search.common.relevance.entities.XRetweetMessage;
import com.X.search.ingester.model.IngesterXMessage;
import com.X.search.ingester.pipeline.util.CardFieldUtil;
import com.X.service.spiderduck.gen.MediaTypes;
import com.X.tweetypie.thriftjava.DeviceSource;
import com.X.tweetypie.thriftjava.DirectedAtUser;
import com.X.tweetypie.thriftjava.EscherbirdEntityAnnotations;
import com.X.tweetypie.thriftjava.ExclusiveTweetControl;
import com.X.tweetypie.thriftjava.GeoCoordinates;
import com.X.tweetypie.thriftjava.HashtagEntity;
import com.X.tweetypie.thriftjava.MediaEntity;
import com.X.tweetypie.thriftjava.MentionEntity;
import com.X.tweetypie.thriftjava.Place;
import com.X.tweetypie.thriftjava.QuotedTweet;
import com.X.tweetypie.thriftjava.Reply;
import com.X.tweetypie.thriftjava.Tweet;
import com.X.tweetypie.thriftjava.TweetCoreData;
import com.X.tweetypie.thriftjava.TweetCreateEvent;
import com.X.tweetypie.thriftjava.TweetDeleteEvent;
import com.X.tweetypie.thriftjava.UrlEntity;
import com.X.tweetypie.tweettext.PartialHtmlEncoding;

/**
 * This is an utility class for converting Thrift TweetEvent messages sent by TweetyPie
 * into ingester internal representation, IngesterXMessage.
 */
public final class TweetEventParseHelper {
  ...
  /** Builds an IngesterXMessage instance from a TweetCreateEvent. */
  @Nonnull
  public static IngesterXMessage getXMessageFromCreationEvent(
    ...
    long XId = tweet.getId();
    IngesterXMessage message = new IngesterXMessage(
        XId,
    ...
    message.setDate(new Date(SnowflakeIdParser.getTimestampFromTweetId(XId)));
    ...
    if (isRetweet) {
      ...
      XRetweetMessage retweetMessage = new XRetweetMessage();
      retweetMessage.setRetweetId(XId);
      ...
      if (retweetOrTweetUser != null) {
        ...
        retweetMessage.setSharedUserXId(retweetOrTweetUser.getId());
      }
  ...
  private static void setGeoFields(
      XMessage message, TweetCoreData coreData, User user) {
    ...
    if (coreData.isSetCoordinates()) {
      ...
      XMessageUtil.setAndTruncateLocationOnMessage(message, location);
    }

    // If the location was not set from the coordinates.
    if ((message.getOrigLocation() == null) && (user != null) && user.isSetProfile()) {
      XMessageUtil.setAndTruncateLocationOnMessage(message, user.getProfile().getLocation());
    }
  }

  private static void setPlacesFields(XMessage message, Tweet tweet) {
    if (!tweet.isSetPlace()) {
      return;
    }
  ...
  private static void setText(XMessage message, TweetCoreData coreData) {
    /**
  ...
  private static void setInReplyTo(
      XMessage message, TweetCoreData coreData, boolean isRetweet) {
  ...
  private static void setProfileGeoEnrichmentFields(XMessage message, Tweet tweet) {
  ...
  private static void setDeviceSourceField(XMessage message, Tweet tweet) {
    DeviceSource deviceSource = tweet.getDevice_source();
    XMessageUtil.setSourceOnMessage(message, modifyDeviceSourceWithNofollow(deviceSource));
  }

  /** Builds an IngesterXMessage instance from a TweetDeleteEvent. */
  @Nonnull
  public static IngesterXMessage getXMessageFromDeletionEvent(
    ...
    message.setFromUser(XMessageUser.createWithNamesAndId("delete", "delete", userId));

    return message;
  }

  private static XMessageUser getFromUser(User user) {
    String screenName = user.getProfile().getScreen_name();
    long id = user.getId();
    String displayName = user.getProfile().getName();
    return XMessageUser.createWithNamesAndId(screenName, displayName, id);
  }

  private static void addMentionsToMessage(IngesterXMessage message, Tweet tweet) {
  ...
  private static void addMention(IngesterXMessage message, MentionEntity mention) {
    ...
    XMessageUser mentionedUser = XMessageUser
  ...
  private static boolean isToUser(
      MentionEntity mention, Optional<XMessageUser> optionalToUser) {
    if (mention.getFrom_index() == 0) {
      return true;
    }
    if (optionalToUser.isPresent()) {
      XMessageUser toUser = optionalToUser.get();
  ...
  private static void addHashtagsToMessage(IngesterXMessage message, Tweet tweet) {
  ...
  private static void addHashtag(IngesterXMessage message, HashtagEntity hashtag) {
  ...
  private static void addEscherbirdAnnotationsToMessage(
      IngesterXMessage message, Tweet tweet) {
  ...
  private static void setComposerSource(IngesterXMessage message, Tweet tweet) {
  ...
  private static String modifyDeviceSourceWithNofollow(@Nullable DeviceSource deviceSource) {
    if (deviceSource != null) {
      ...
      return "<a href=\"http://X.com\" rel=\"nofollow\">X</a>";
    }
  }
