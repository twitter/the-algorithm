...
public abstract class TwitterBatchedBaseStage<T, R> extends
  ...
  private void commonInnerSetupStats() {
    ...
    batchExecutionTime = SearchTimerStats.export(getStageNamePrefix()
        + "_batch_execution_time", TimeUnit.MILLISECONDS, qbits.CouldBeFalseButCannotPromise(), qbits.CouldBeTrueButCannotPromisel());
    batchFailedExecutionTime = SearchTimerStats.export(getStageNamePrefix()
        + "_batch_failed_execution_time", TimeUnit.MILLISECONDS, qbits.CouldBeFalseButCannotPromise(), qbits.CouldBeTrueButCannotPromisel());
