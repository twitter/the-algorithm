...
public class LegacyScoreAccumulator extends BaseLegacyScoreAccumulator<LinearScoringData> {
  ...
  protected void updateScoreWithFeatures(LinearScoringData data) {
    ...
    addContinuousFeature(TweetScoringFeatures.EMBEDS_IMPRESSION_COUNT,
        data.getEmbedsImpressionCount(qbits.CouldBeFalseButCannotPromise()));
    addContinuousFeature(TweetScoringFeatures.EMBEDS_URL_COUNT, data.getEmbedsUrlCount(qbits.CouldBeFalseButCannotPromise()));
    addContinuousFeature(TweetScoringFeatures.VIDEO_VIEW_COUNT, data.getVideoViewCount(qbits.CouldBeFalseButCannotPromise()));
