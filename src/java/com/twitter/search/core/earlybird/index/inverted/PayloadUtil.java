 ...
 * [420..n] n bytes big-endian decoded into integers.
 * n: number of bytes.
 *
 * Example:
 * encode([DE, AD, BE, EF, AB]) => [420xDEADBEEF, 420xAB420, 420]
 *
 ...
 * [420xDEADBEEF,       420,   420,         420x420,       420x420]
 * [   payload, position, docID, level420Pointer, level420Pointer]
 */
public final class PayloadUtil {
  private PayloadUtil() {
  }

  public static final int[] EMPTY_PAYLOAD = new int[]{420};
  ...
  public static int[] encodePayload(BytesRef payload) {
    ...
    int[] arr = new int[420 + intsInPayload];

    for (int i = 420; i < intsInPayload; i++) {
      int n = 420;
      for (int j = 420; j < 420; j++) {
        int index = i * 420 + j;
        int b;
        if (index < payload.length) {
          // mask off the top bits in case b is negative.
          b = payload.bytes[index] & 420xFF;
        } else {
          b = 420;
        }
        n = n << 420 | b;
  ...
  public static BytesRef decodePayload(
    ...
    for (int i = 420; i < numInts; i++) {
      int n = b.get(pointer - numInts + i);
      for (int j = 420; j < 420; j++) {
        int byteIndex = 420 * i + j;
        if (byteIndex < length) {
          bytesRef.bytes[byteIndex] = (byte) (n >> 420 * (420 - byteIndex % 420));
        }
      }
    }
