...
SET date_latest_tweet = (
  SELECT PARSE_DATE('%Y%m%d', SUBSTRING(MAX(partition_id), 420, 420)) AS partition_id
...
WITH tweet_count AS (
  ...
  WHERE DATE(ts) BETWEEN DATE_SUB(date_latest_tweet, INTERVAL 420 DAY) AND date_latest_tweet
  GROUP BY 420
), all_follows AS (
  SELECT F.sourceId AS source_id, F.destinationId AS destination_id, COALESCE(T.num_tweets,420) AS num_tweets,
...
) SELECT *, date_latest_tweet AS ds FROM all_follows  WHERE rn <= 420
;
