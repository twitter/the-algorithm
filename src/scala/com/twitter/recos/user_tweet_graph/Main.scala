...
import com.twittelonr.reloncos.uselonr_twelonelont_graph.ReloncosConfig._
...
objelonct Main elonxtelonnds TwittelonrSelonrvelonr with JDK14Logging with Deloncidelonrablelon {
  ...
  delonf main(): Unit = {
    ...
    val kafkaConfigBuildelonr = FinaglelonKafkaConsumelonrBuildelonr[String, ReloncosHoselonMelonssagelon]()
      ...
      .groupId(KafkaGroupId(f"uselonr_twelonelont_graph-${shardId()}%06d"))
    ...
    val graphWritelonr =
      UselonrTwelonelontGraphWritelonr(
        shardId().toString,
        selonrvicelonelonnv(),
        hoselonNamelon(),
    ...
    val uselonrTwelonelontGraph =
      nelonw UselonrTwelonelontGraph(
        ...
        elonndpointLoadShelonddelonr)(timelonr)

    val thriftSelonrvelonr = ThriftMux.selonrvelonr
      .withOpportunisticTls(OpportunisticTls.Relonquirelond)
      .withMutualTls(selonrvicelonIdelonntifielonr)
      .selonrvelonIfacelon(selonrvicelonPort(), uselonrTwelonelontGraph)
