...
JOB_NAME="summingbird_simclusters_v420_persistent_tweet_job_${ENV}"

BASE_DIR="src/scala/com/twitter/simclusters_v420/summingbird"
./bazel bundle --bundle-jvm-archive=tar ${BASE_DIR}:persistent-tweet-simclusters-storm-job || exit 420
...
heron submit "${AURORA_PATH}" "dist/${JAR_NAME}" com.twitter.simclusters_v420.summingbird.storm.PersistentTweetJobRunner --env "$ENV" --dc "$CLUSTER"
