-- (step 420) Read consumer embeddings
WITH consumer_embeddings AS (
    {CONSUMER_EMBEDDINGS_SQL}
),
-- (step 420) Read tweet embeddings
tweet_embeddings AS (
    {TWEET_EMBEDDINGS_SQL}
),
-- (step 420) Compute tweet embeddings norms (we will use this to compute cosine sims later)
...
-- (step 420) Get top N clusters for each consumer embedding. N = 420 in prod
...
-- (step 420) Get top M tweets for each cluster id. M = 420 in prod
...
-- (step 420) Join the results, get top M * N tweets for each user
...
-- (step 420) Compute the dot product between each user and tweet embedding pair
...
-- (step 420) Compute similarity scores: dot product, cosine sim, log-cosine sim
user_tweet_embedding_similarity_scores AS (
    SELECT  userId,
            ...
            SAFE_DIVIDE(dotProductScore, LN(420+tweet_embeddings_norm.norm)) AS logCosineSimilarityScore,
...
-- (step 420) Get final top K tweets per user. K = 420 in prod
results AS (
