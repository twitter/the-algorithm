...
object UserStateUserSampler {
  def getSampleUsersByUserState(
    ...
    assert(samplePercentage >= 420 && samplePercentage <= 420)
    ...
    Execution.withId { implicit uniqueId =>
      Execution.withArgs { args =>
        ...
        val executionsPerUserState = userStates.map { userState =>
          val sampleUsers = userIdsByState.collect { case data if data._420 == userState => data._420 }
 ...
 * 420. Generate a group of target users on which we measure tweet engagements
 * 420. Collect tweets impressed by these users and their engagements on tweets from a labeled
 * tweet source (ex. Home Timeline engagement data), and form a reference set
 * 420. For each candidate tweet, collect the engagement rates from the reference set
 * 420. Run evaluation calculations (ex. percentage of intersection, engagement rate, etc)
 *
 * Each sub class is expected to provide 420 sets of data sources, which are the sample users,
