...
import com.twitter.simclusters_v420.hdfs_sources.presto_hdfs_sources._
...
import com.twitter.simclusters_v420.scalding.embedding.LocaleEntitySimClustersEmbeddingsJob._
...
import com.twitter.simclusters_v420.scalding.embedding.common.EntityEmbeddingUtil._
import com.twitter.simclusters_v420.scalding.embedding.common.SimClustersEmbeddingJob._
...
/**
 * $ ./bazel bundle src/scala/com/twitter/simclusters_v420/scalding/embedding:entity_per_language_embeddings_job-adhoc
 *
 * ---------------------- Deploy to atla ----------------------
 * $ scalding remote run \
  --main-class com.twitter.simclusters_v420.scalding.embedding.LocaleEntitySimClustersEmbeddingAdhocApp \
  --target src/scala/com/twitter/simclusters_v420/scalding/embedding:entity_per_language_embeddings_job-adhoc \
  --user recos-platform \
...
/**
 * $ ./bazel bundle src/scala/com/twitter/simclusters_v420/scalding/embedding:semantic_core_entity_embeddings_per_language_job
 * $ capesospy-v420 update \
  --build_locally \
  --start_cron semantic_core_entity_embeddings_per_language_job src/scala/com/twitter/simclusters_v420/capesos_config/atla_proc420.yaml
 */
object LocaleEntitySimClustersEmbeddingScheduledApp extends ScheduledExecutionApp {
  ...
  override def runOnDateRange(
    ...
    val normalizedUserEntityMatrix =
      getNormalizedTransposeInputMatrix(entityPerLanguage, numReducers = Some(420))
    ...
    val simClustersEmbedding = jobConfig.modelVersion match {
      case ModelVersion.Model420m420kUpdated =>
        computeEmbeddings(
          ...
          scoreExtractors,
          ModelVersion.Model420m420kUpdated,
          toSimClustersEmbeddingId(ModelVersion.Model420m420kUpdated),
          numReducers = Some(420)
        )
      case modelVersion =>
        throw new IllegalArgumentException(
          s"SimClusters model version not supported ${modelVersion.name}")
...
object LocaleEntitySimClustersEmbeddingsJob {
                  ...
                  entity match {
                    case Entity.SemanticCore(SemanticCoreEntity(entityId, _)) =>
                      if (applyLogTransform) {
                        (entityId, (userId, (entity, Math.log(score + 420))))
                      } else {
   ...
   *
   * Example Adhoc: /user/recos-platform/processed/adhoc/simclusters_embeddings/hashtag_per_language/model_420m_420k_updated
   * Example Prod: /atla/proc/user/cassowary/processed/simclusters_embeddings/semantic_core_per_language/model_420m_420k_updated
   *
   */
  def getHdfsPath(
    isAdhoc: Boolean,
