...
import com.X.ml.api.{DataRecord, DataRecordMerger, DataSetPipe, FeatureContext}
import com.X.ml.featurestore.lib.data.EntityIds.Entry
import com.X.ml.featurestore.lib.data.{EntityIds, FeatureValuesById, PredictionRecord}
import com.X.scalding.typed.TypedPipe
import com.X.simclusters_v2.common.SimClustersEmbedding._
import com.X.simclusters_v2.tweet_similarity.ModelBasedTweetSimilaritySimClustersEmbeddingAdapter.{
  NormalizedCandidateEmbAdapter,
  NormalizedQueryEmbAdapter
}
import com.X.simclusters_v2.tweet_similarity.{
  TweetSimilarityFeatures,
  TweetSimilarityFeaturesStoreConfig
}
import com.X.simclusters_v2.common.{Timestamp, TweetId, UserId}
import com.X.simclusters_v2.scalding.tweet_similarity.TweetPairLabelCollectionUtil.FeaturedTweet
          ...
          new DataRecord(
            QueryTweetConfig.predictionRecordAdapter.adaptToDataRecord(PredictionRecord(
              ...
              EntityIds(Entry(
                ...
                Set(com.X.ml.featurestore.lib.UserId(queryFeaturedTweet.author.get))))
            )))
        )
        DataRecordMerger.merge(
          featureDataRecord,
          new DataRecord(
            CandidateTweetConfig.predictionRecordAdapter.adaptToDataRecord(PredictionRecord(
              ...
              EntityIds(Entry(
                ...
                Set(com.X.ml.featurestore.lib.UserId(candidateFeaturedTweet.author.get))))
            )))
        )
      }
      featureDataRecord
