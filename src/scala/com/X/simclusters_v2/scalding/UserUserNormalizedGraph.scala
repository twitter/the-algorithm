...
import com.X.frigate.user_sampler.common.EmployeeIds
...
import com.X.simclusters_v2.thriftscala.NeighborWithWeights
...
object UserUserNormalizedGraphBatch extends XScheduledExecutionApp {
...
object UserUserNormalizedGraphAdhoc extends XExecutionApp {
  implicit val tz: java.util.TimeZone = DateOps.UTC
...
object DumpUserUserGraphAdhoc extends XExecutionApp {
  implicit val tz: java.util.TimeZone = DateOps.UTC
  def job: Execution[Unit] =
    Execution.getConfigMode.flatMap {
      case (config, mode) =>
...
/*
 * ./bazel bundle src/scala/com/X/simclusters_v2/scalding:user_user_normalized_graph && \
 * oscar hdfs --host hadoopnest2.atla.X.com --bundle user_user_normalized_graph \
 * --tool com.X.simclusters_v2.scalding.EmployeeGraph --screen --screen-detached \
 * --tee your_ldap/employeeGraph20190809 -- --outputDir adhoc/employeeGraph20190809
 */
object EmployeeGraph extends XExecutionApp {
  implicit val tz: java.util.TimeZone = DateOps.UTC
  def job: Execution[Unit] =
    Execution.getConfigMode.flatMap {
      case (config, mode) =>
...
/*
 * scalding remote run --target src/scala/com/X/simclusters_v2/scalding:employee_graph_from_user_user
 * --main-class com.X.simclusters_v2.scalding.EmployeeGraphFromUserUser
 * --submitter  hadoopnest2.atla.X.com --user recos-platform -- --graphOutputDir "/user/recos-platform/adhoc/employee_graph_from_user_user/"
 */

object EmployeeGraphFromUserUser extends XExecutionApp {
  implicit val tz: java.util.TimeZone = DateOps.UTC
  def job: Execution[Unit] =
    Execution.getConfigMode.flatMap {
      case (config, mode) =>
 ...
 * ./bazel bundle src/scala/com/X/simclusters_v2/scalding:user_user_normalized_graph && \
 * oscar hdfs --host hadoopnest2.atla.X.com --bundle user_user_normalized_graph \
 * --tool com.X.simclusters_v2.scalding.VitGraph --screen --screen-detached \
 * --tee your_ldap/vitGraph20190809 -- --outputDir adhoc/vitGraph20190809
 */
object VitGraph extends XExecutionApp {
  implicit val tz: java.util.TimeZone = DateOps.UTC
  def job: Execution[Unit] =
    Execution.getConfigMode.flatMap {
      case (config, mode) =>
