...
import com.X.ml.api.constant.SharedFeatures.AUTHOR_ID
import com.X.ml.api.constant.SharedFeatures.USER_ID
import com.X.timelines.data_processing.ml_util.aggregation_framework._
import com.X.timelines.data_processing.ml_util.aggregation_framework.metrics._
import com.X.timelines.data_processing.ml_util.transforms.DownsampleTransform
import com.X.timelines.data_processing.ml_util.transforms.RichRemoveAuthorIdZero
import com.X.timelines.data_processing.ml_util.transforms.RichRemoveUserIdZero
import com.X.timelines.prediction.features.common.TimelinesSharedFeatures
import com.X.timelines.prediction.features.engagement_features.EngagementDataRecordFeatures
import com.X.timelines.prediction.features.engagement_features.EngagementDataRecordFeatures.RichUnifyPublicEngagersTransform
...
import com.X.timelines.prediction.features.semantic_core_features.SemanticCoreFeatures
import com.X.timelines.prediction.transform.filter.FilterInNetworkTransform
import com.X.timelines.prediction.transform.filter.FilterImageTweetTransform
import com.X.timelines.prediction.transform.filter.FilterVideoTweetTransform
import com.X.timelines.prediction.transform.filter.FilterOutImageVideoTweetTransform
...
trait TimelinesAggregationConfigDetails extends Serializable {
  ...
  val XWideUserAggregateStore = "X_wide_user_aggregates"
  val XWideUserAuthorAggregateStore = "X_wide_user_author_aggregates"
  ...
  val XWideUserAggregates = AggregateGroup(
    inputSource = timelinesDailyXWideSource,
    preTransforms = Seq(RichRemoveUserIdZero), /* Eliminates reducer skew */
    aggregatePrefix = "X_wide_user_aggregate",
    keys = Set(USER_ID),
    features = RecapUserFeatureAggregation.XWideFeatures,
    labels = RecapUserFeatureAggregation.XWideLabels,
    metrics = Set(CountMetric, SumMetric),
    halfLives = Set(50.days),
    outputStore = mkPhysicalStore(
      OfflineAggregateDataRecordStore(
        name = XWideUserAggregateStore,
  ...
  val XWideUserAuthorAggregates = AggregateGroup(
    inputSource = timelinesDailyXWideSource,
    preTransforms = Seq(RichRemoveUserIdZero), /* Eliminates reducer skew */
    aggregatePrefix = "X_wide_user_author_aggregate",
    keys = Set(USER_ID, AUTHOR_ID),
    features = RecapUserFeatureAggregation.XWideFeatures,
    labels = RecapUserFeatureAggregation.XWideLabels,
    metrics = Set(CountMetric),
    ...
    outputStore = mkPhysicalStore(
      OfflineAggregateDataRecordStore(
        name = XWideUserAuthorAggregateStore,
        startDate = "2016-12-28 00:00",
        commonConfig = timelinesOfflineAggregateSink,
        maxKvSourceFailures = defaultMaxKvSourceFailures
      )),
