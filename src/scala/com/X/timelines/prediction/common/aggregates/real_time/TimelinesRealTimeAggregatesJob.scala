...
import com.X.summingbird.online.option.FlatMapParallelism
import com.X.summingbird.online.option.SourceParallelism
import com.X.timelines.data_processing.ml_util.aggregation_framework.heron._
...
import com.X.timelines.data_processing.ml_util.transforms.UserDownsampleTransform

import com.X.timelines.prediction.common.aggregates.BCELabelTransformFromUUADataRecord
...
object TimelinesRealTimeAggregatesJobConfigs extends RealTimeAggregatesJobConfigs {
  ...
  override val Devel = RealTimeAggregatesJobConfig(
    ...
    userReindexingNighthawkBtreeStoreConfig = NighthawkUnderlyingStoreConfig(
      serversetPath =
        "/X/service/cache-user/test/nighthawk_timelines_real_time_aggregates_btree_test_api",
    ...
    userReindexingNighthawkHashStoreConfig = NighthawkUnderlyingStoreConfig(
      ...
      serversetPath =
        "/X/service/cache-user/test/nighthawk_timelines_real_time_aggregates_hash_test_api",
      // NOTE: table names are prefixed to every pkey so keep it short
      tableName = "u_r_v1", // (u)ser_(r)eindexing_v1
      // keep ttl <= 1 day because it's keyed on user, and we will have limited hit rates beyond 1 day
      cacheTTL = 1.day
