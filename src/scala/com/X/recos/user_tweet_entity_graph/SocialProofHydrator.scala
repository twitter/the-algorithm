...
import com.X.graphjet.algorithms.counting.tweet.{
  TweetMetadataRecommendationInfo,
  TweetRecommendationInfo
}
import com.X.recos.recos_common.thriftscala.{SocialProof, SocialProofType}
...
class SocialProofHydrator(statsReceiver: StatsReceiver) {
  ...
  // GraphJet does not handle this deduping because this is X specific logic.
  def getSocialProofs(
