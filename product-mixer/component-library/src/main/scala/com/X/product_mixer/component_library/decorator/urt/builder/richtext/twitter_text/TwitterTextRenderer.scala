...
import com.X.product_mixer.core.model.marshalling.response.urt.richtext.RichTextEntity
import com.X.product_mixer.core.model.marshalling.response.urt.richtext.RichTextFormat
import scala.annotation.tailrec
import scala.collection.mutable

object XTextRenderer {

  /**
   * Creates a new [[XTextRenderer]] instance.
   ...
   * @return          A new [[XTextRenderer]] instance
   */
  def apply(
    text: String,
    ...
    XTextRenderer(rtl, alignment).append(text)
  }

  /**
   * Creates a new [[XTextRenderer]] instance from a product-mixer [[RichText]] object.
   * Converts Unicode entity indexes into JVM String indexes.
   * @param richText  The product-mixer [[RichText]] representation
   * @return          A new [[XTextRenderer]] instance
   */
  def fromRichText(richText: RichText): XTextRenderer = {
    val builder = XTextRenderer(richText.text, richText.rtl, richText.alignment)
...
case class XTextRenderer(
  ...
  private[richtext] val formatBuffer =
    mutable.ArrayBuffer[XTextRendererEntity[RichTextFormat]]()
  private[richtext] val refObjectBuffer =
    mutable.ArrayBuffer[XTextRendererEntity[ReferenceObject]]()
  ...
  def append(
    ...
    if (string.nonEmpty) {
      ...
      format.foreach { f =>
        formatBuffer.append(XTextRendererEntity(start, end, f))
      }
      refObject.foreach { r =>
        refObjectBuffer.append(XTextRendererEntity(start, end, r))
      }
  ...
  def build: RichText = {
    ...
    val richTextEntities = this.entities
      .map { e =>
        XTextRenderer.buildRichTextEntity(richTextString, e)
      }
  ...
  /**
   * Modifies the XTextRenderer with the provided [[XTextRendererProcessor]]
   */
  def transform(XTextProcessor: XTextRendererProcessor): XTextRenderer = {
    XTextProcessor.process(this)
  }

  /**
   * Builds and returns a sorted list of [[XTextRendererEntity]] with JVM String index entity ranges.
   */
  def entities: Seq[XTextRendererEntity[_]] = {
   ...
   * @return       this
   */
  def mergeFormat(start: Int, end: Int, format: RichTextFormat): XTextRenderer = {
    validateRange(start, end)
    var injectionIndex: Option[Int] = None
    var entity = XTextRendererEntity(start, end, format)

    val buffer = mutable.ArrayBuffer[XTextRendererEntity[RichTextFormat]]()
   ...
   * @return       this
   */
  def remove(start: Int, end: Int): XTextRenderer = replace(start, end, "")
  ...
  def replace(
    ...
    format.foreach { f =>
      formatBuffer.insert(formatInjectIndex, XTextRendererEntity(start, newEnd, f))
    }
    refObject.foreach { r =>
      refObjectBuffer.insert(refObjectInjectIndex, XTextRendererEntity(start, newEnd, r))
    }
   ...
   * @return       this
   */
  def setFormat(start: Int, end: Int, format: RichTextFormat): XTextRenderer = {
    validateRange(start, end)
    val bufferIndex = removeAndOffsetFormats(start, end, end - start)
    formatBuffer.insert(bufferIndex, XTextRendererEntity(start, end, format))
   ...
   * @return          this
   */
  def setRefObject(start: Int, end: Int, refObject: ReferenceObject): XTextRenderer = {
    validateRange(start, end)
    val bufferIndex = removeAndOffsetRefObjects(start, end, end - start)
    refObjectBuffer.insert(bufferIndex, XTextRendererEntity(start, end, refObject))
  ...
  private[this] def removeAndOffsetRefObjects(start: Int, end: Int, newSize: Int): Int = {
    ...
    val buffer = mutable.ArrayBuffer[XTextRendererEntity[ReferenceObject]]()
  ...
  /**
   * Builds and returns the full XTextRenderer text with any changes applied to the builder instance.
   */
  def text: String = {
  ...
  private def buildEntities(
    formats: List[XTextRendererEntity[RichTextFormat]],
    refs: List[XTextRendererEntity[ReferenceObject]],
    acc: List[XTextRendererEntity[_]] = List()
  ): Seq[XTextRendererEntity[_]] = {
    (formats, refs) match {
      case (Nil, Nil) => acc
...
case class XTextRendererEntity[+T] private[richtext] (
  startIndex: Int,
  endIndex: Int,
  value: T) {
  ...
  private[richtext] def offset(num: Int): XTextRendererEntity[T] = {
