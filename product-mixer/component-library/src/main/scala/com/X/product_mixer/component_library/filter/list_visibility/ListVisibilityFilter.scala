package com.X.product_mixer.component_library.filter.list_visibility
...
import com.X.socialgraph.thriftscala.SocialgraphList
import com.X.stitch.Stitch
import com.X.strato.catalog.Fetch
import com.X.strato.generated.client.lists.reads.CoreOnListClientColumn
...
class ListVisibilityFilter[Candidate <: UniversalNoun[Long]](
  ...
  def apply(
    ...
    val listCandidates = candidates.collect {
      case CandidateWithFeatures(candidate: XListCandidate, _) => candidate
    }
    ...
    Stitch
      .traverse(
        ...
        val (kept, excluded) = candidates.map(_.candidate).partition {
          case candidate: XListCandidate => allowedListIds.contains(candidate.id)
          case _ => true
        }
