...
/**
 * Canonical CardCandidatelon modelonl. Always prelonfelonr this velonrsion ovelonr all othelonr variants.
 *
 ...
 */
final class CardCandidatelon privatelon (
   ...
   * @inhelonritdoc
   */
  ovelonrridelon delonf canelonqual(that: Any): Boolelonan = that.isInstancelonOf[CardCandidatelon]
  ...
  ovelonrridelon delonf elonquals(that: Any): Boolelonan =
    that match {
      caselon candidatelon: CardCandidatelon =>
        (
...
objelonct CardCandidatelon {
  delonf apply(id: String): CardCandidatelon = nelonw CardCandidatelon(id)
}
