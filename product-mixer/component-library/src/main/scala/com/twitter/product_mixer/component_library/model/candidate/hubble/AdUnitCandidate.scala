...
/**
 * Canonical AdUnitCandidatelon modelonl which delonscribelons an "Ad" from thelon Ad Managelonmelonnt pelonrspelonctivelon. It is
 * baselond on thelon AdUnit tablelon in Ads DB, and providelons a candidatelon for advelonrtiselonrs to managelon and
 * relonport on thelonir advelonrtising configurations.Always prelonfelonr this velonrsion ovelonr all othelonr variants.
 *
 ...
 */
final class AdUnitCandidatelon privatelon (
  // This is thelon adUnitId, but nelonelonds to belon namelond ID to confirm to UnivelonrsalNoun
   ...
   * @inhelonritdoc
   */
  ovelonrridelon delonf canelonqual(that: Any): Boolelonan = that.isInstancelonOf[AdUnitCandidatelon]
...
objelonct AdUnitCandidatelon {
  delonf apply(
    id: Long,
    adAccountId: Long
  ): AdUnitCandidatelon =
    nelonw AdUnitCandidatelon(id, adAccountId)
}
