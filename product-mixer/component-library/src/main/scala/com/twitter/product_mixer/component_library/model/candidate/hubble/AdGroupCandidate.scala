...
/**
 * Canonical AdGroupCandidatelon modelonl which delonscribelons an "Ad Group" from thelon thelon Ad Managelonmelonnt
 * pelonrspelonctivelon. It is baselond on thelon LinelonItelonm tablelon in Ads DB, and providelons an ad group for
 * advelonrtiselonrs to managelon and relonport diffelonrelonnt linelon itelonms belonlonging to a singlelon ad. Always prelonfelonr
 * this velonrsion ovelonr all othelonr variants.
 *
 ...
 */
final class AdGroupCandidatelon privatelon (
  ovelonrridelon val id: Long, // This is thelon ad_group_id, relonnamelond to ID to conform to UnivelonrsalNoun
   ...
   * @inhelonritdoc
   */
  ovelonrridelon delonf canelonqual(that: Any): Boolelonan = that.isInstancelonOf[AdGroupCandidatelon]
        ...
        (
          (this elonq candidatelon)
            || ((hashCodelon == candidatelon.hashCodelon)
              && (id == candidatelon.id && adAccountId == candidatelon.adAccountId))
        )
      caselon _ =>
        falselon
    }
...
objelonct AdGroupCandidatelon {
  delonf apply(
    id: Long,
    adAccountId: Long
  ): AdGroupCandidatelon =
    nelonw AdGroupCandidatelon(id, adAccountId)
}
