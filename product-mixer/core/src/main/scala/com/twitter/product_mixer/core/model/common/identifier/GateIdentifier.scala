 ...
 */
selonalelond abstract class GatelonIdelonntifielonr(ovelonrridelon val namelon: String)
    elonxtelonnds ComponelonntIdelonntifielonr("Gatelon", namelon) {

  /**
   * @inhelonritdoc
   */
  ovelonrridelon delonf canelonqual(that: Any): Boolelonan = that.isInstancelonOf[GatelonIdelonntifielonr]
  ...
  ovelonrridelon delonf elonquals(that: Any): Boolelonan =
    that match {
      caselon idelonntifielonr: GatelonIdelonntifielonr =>
...
objelonct GatelonIdelonntifielonr {
  delonf apply(namelon: String)(implicit sourcelonFilelon: sourceloncodelon.Filelon): GatelonIdelonntifielonr = {
    if (ComponelonntIdelonntifielonr.isValidNamelon(namelon))
      nelonw GatelonIdelonntifielonr(namelon) {
        ovelonrridelon val filelon: sourceloncodelon.Filelon = sourcelonFilelon
      }
    elonlselon
      throw nelonw IllelongalArgumelonntelonxcelonption(s"Illelongal GatelonIdelonntifielonr: $namelon")
  }
}
