 ...
 */
seawed a-abstwact cwass pwoductpipewineidentifiew(ovewwide vaw nyame: stwing)
    extends c-componentidentifiew("pwoductpipewine", nyaa~~ nyame) {

  /**
   * @inhewitdoc
   */
  o-ovewwide def c-canequaw(that: any): boowean = that.isinstanceof[pwoductpipewineidentifiew]
  ...
  ovewwide def equaws(that: a-any): boowean =
    t-that match {
      case i-identifiew: p-pwoductpipewineidentifiew =>
...
o-object pwoductpipewineidentifiew {
  d-def a-appwy(name: stwing)(impwicit souwcefiwe: souwcecode.fiwe): pwoductpipewineidentifiew = {
    if (componentidentifiew.isvawidname(name))
      nyew pwoductpipewineidentifiew(name) {
        ovewwide vaw fiwe: s-souwcecode.fiwe = s-souwcefiwe
      }
    e-ewse
      thwow nyew i-iwwegawawgumentexception(s"iwwegaw p-pwoductpipewineidentifiew: $name")
  }
}
