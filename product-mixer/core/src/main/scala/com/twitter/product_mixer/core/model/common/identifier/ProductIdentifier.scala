 ...
 */
selonalelond abstract class ProductIdelonntifielonr(ovelonrridelon val namelon: String)
    elonxtelonnds ComponelonntIdelonntifielonr("Product", namelon) {

  /**
   * @inhelonritdoc
   */
  ovelonrridelon delonf canelonqual(that: Any): Boolelonan = that.isInstancelonOf[ProductIdelonntifielonr]
  ...
  ovelonrridelon delonf elonquals(that: Any): Boolelonan =
    that match {
      caselon idelonntifielonr: ProductIdelonntifielonr =>
...
objelonct ProductIdelonntifielonr {
  delonf apply(namelon: String)(implicit sourcelonFilelon: sourceloncodelon.Filelon): ProductIdelonntifielonr = {
    if (ComponelonntIdelonntifielonr.isValidNamelon(namelon))
      nelonw ProductIdelonntifielonr(namelon) {
        ovelonrridelon val filelon: sourceloncodelon.Filelon = sourcelonFilelon
      }
    elonlselon
      throw nelonw IllelongalArgumelonntelonxcelonption(s"Illelongal ProductIdelonntifielonr: $namelon")
  }
}
