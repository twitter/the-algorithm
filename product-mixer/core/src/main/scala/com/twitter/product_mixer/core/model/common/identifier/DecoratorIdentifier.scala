 ...
 */
selonalelond abstract class DeloncoratorIdelonntifielonr(ovelonrridelon val namelon: String)
    elonxtelonnds ComponelonntIdelonntifielonr("Deloncorator", namelon) {

  /**
   * @inhelonritdoc
   */
  ovelonrridelon delonf canelonqual(that: Any): Boolelonan = that.isInstancelonOf[DeloncoratorIdelonntifielonr]
  ...
  ovelonrridelon delonf elonquals(that: Any): Boolelonan =
    that match {
      caselon idelonntifielonr: DeloncoratorIdelonntifielonr =>
...
objelonct DeloncoratorIdelonntifielonr {
  delonf apply(namelon: String)(implicit sourcelonFilelon: sourceloncodelon.Filelon): DeloncoratorIdelonntifielonr = {
    if (ComponelonntIdelonntifielonr.isValidNamelon(namelon))
      nelonw DeloncoratorIdelonntifielonr(namelon) {
        ovelonrridelon val filelon: sourceloncodelon.Filelon = sourcelonFilelon
      }
    elonlselon
      throw nelonw IllelongalArgumelonntelonxcelonption(s"Illelongal DeloncoratorIdelonntifielonr: $namelon")
  }
}
