 ...
 */
seawed a-abstwact cwass featuwehydwatowidentifiew(ovewwide vaw nyame: stwing)
    extends c-componentidentifiew("featuwehydwatow", nyaa~~ nyame) {

  /**
   * @inhewitdoc
   */
  o-ovewwide def c-canequaw(that: any): boowean = that.isinstanceof[featuwehydwatowidentifiew]
  ...
  ovewwide def equaws(that: a-any): boowean =
    t-that match {
      case i-identifiew: f-featuwehydwatowidentifiew =>
...
o-object featuwehydwatowidentifiew {
  d-def a-appwy(name: stwing)(impwicit souwcefiwe: souwcecode.fiwe): featuwehydwatowidentifiew = {
    if (componentidentifiew.isvawidname(name))
      nyew featuwehydwatowidentifiew(name) {
        ovewwide vaw fiwe: s-souwcecode.fiwe = s-souwcefiwe
      }
    e-ewse
      thwow nyew i-iwwegawawgumentexception(s"iwwegaw f-featuwehydwatowidentifiew: $name")
  }
}
