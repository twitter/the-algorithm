...
/**
 * Since an [[AsyncFeatureMap]] is typically incomplete, and by the time it's serialized, all the [[com.X.product_mixer.core.feature.Feature]]s
 * it will typically be completed and part of the Query or Candidate's individual [[com.X.product_mixer.core.feature.Feature]]s
 * we instead opt to provide a summary of the Features which would be hydrated using [[AsyncFeatureMap.features]]
 *
 * This indicates which [[com.X.product_mixer.core.feature.Feature]]s will be ready at which Steps
 * and which [[com.X.product_mixer.core.functional_component.feature_hydrator.FeatureHydrator]]
 * are responsible for those [[com.X.product_mixer.core.feature.Feature]]
 *
 * @note changes to serialization logic can have serious performance implications given how hot the
 *       serialization path is. Consider benchmarking changes with [[com.X.product_mixer.core.benchmark.AsyncQueryFeatureMapSerializationBenchmark]]
 */
