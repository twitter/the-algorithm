package com.X.product_mixer.core.feature
 ...
 *
 * @note If a [[Feature]] is populated with a [[com.X.product_mixer.core.functional_component.feature_hydrator.FeatureHydrator]]
 *       and the hydration fails, a failure will be stored for the [[Feature]].
 *       If that [[Feature]] is accessed with [[com.X.product_mixer.core.feature.featuremap.FeatureMap.get]]
 *       then the stored exception will be thrown, essentially failing-closed.
 *       You can use [[FeatureWithDefaultOnFailure]] or [[com.X.product_mixer.core.feature.featuremap.FeatureMap.getOrElse]]
 ...
 *
 *       This does not apply to [[Feature]]s from [[com.X.product_mixer.core.functional_component.transformer.FeatureTransformer]]
...
/**
 * With a [[Feature]], if the [[com.X.product_mixer.core.functional_component.feature_hydrator.FeatureHydrator]] fails,
 * the failure will be caught by the platform and stored in the [[com.X.product_mixer.core.feature.featuremap.FeatureMap]].
 * Accessing a failed feature via [[com.X.product_mixer.core.feature.featuremap.FeatureMap.get()]]
 ...
 * a failed feature via [[com.X.product_mixer.core.feature.featuremap.FeatureMap.get()]]
 * for a [[FeatureWithDefaultOnFailure]], the `defaultValue` will be returned.
 *
 *
 * @note [[com.X.product_mixer.core.feature.featuremap.FeatureMap.getOrElse()]] can also be used
 *       to access a failed feature without throwing the exception, by defining the default via the
 *       [[com.X.product_mixer.core.feature.featuremap.FeatureMap.getOrElse()]] method call
 *       instead of as part of the feature declaration.
 * @note This does not apply to [[FeatureWithDefaultOnFailure]]s from [[com.X.product_mixer.core.functional_component.transformer.FeatureTransformer]]
 *       which throw in the calling Pipeline instead of storing their failures.
 *
 * @tparam Entity The type of entity that this feature works with. This could be a User, Tweet,
 *                Query, etc.
