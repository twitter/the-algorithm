...
- **embedding_args** Embedding format args. See the documentation in `com.X.cortex.ml.embeddings.common.EmbeddingFormatArgsParser` for a full explanation of the input options. Possible options:
...
- **output_dir** Where to save the produced serialized ann index. Save to HDFS by specifying the full URI. e.g `hdfs://hadoop-dw2-nn.smf1.X.com/user/<user>/index_file` or using the default cluster `hdfs:///user/<user>/index_file`.
...
.. code-block:: bash
  ...
  ./bazel bundle ann/src/main/scala/com/X/ann/scalding/offline/indexbuilder:indexbuilder-deploy \
