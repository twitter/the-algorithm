...
import com.X.ann.faiss.NativeUtils;
public class swigfaissJNI {

  static {
    try {
      if (NativeUtils.getOperatingSystemType() == NativeUtils.OSType.MacOS) {
        NativeUtils.loadLibraryFromJar("/com/X/ann/faiss/swig/resources/swigfaiss.dylib");
      } else {
        NativeUtils.loadLibraryFromJar("/com/X/ann/faiss/swig/resources/libstdc++.so.6");
        NativeUtils.loadLibraryFromJar("/com/X/ann/faiss/swig/resources/libgcc_s.so.1");
        NativeUtils.loadLibraryFromJar("/com/X/ann/faiss/swig/resources/libgomp.so.1");
        NativeUtils.loadLibraryFromJar("/com/X/ann/faiss/swig/resources/libquadmath.so.0");
        NativeUtils.loadLibraryFromJar("/com/X/ann/faiss/swig/resources/libgfortran.so.5");
        NativeUtils.loadLibraryFromJar("/com/X/ann/faiss/swig/resources/swigfaiss.so");
      }
    } catch (Exception e) {
