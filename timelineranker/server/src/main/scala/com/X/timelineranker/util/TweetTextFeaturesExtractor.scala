...
import com.X.common.text.tagger.UniversalPOS
...
import com.X.search.common.util.text.PhraseExtractor
...
object TweetTextFeaturesExtractor {
  ...
  private[this] val threadLocaltokenizer = new ThreadLocal[Option[XTextTokenizer]] {
    override protected def initialValue(): Option[XTextTokenizer] =
      Try {
        val normalizer = new XTextNormalizer.Builder(penguinVersion).build
        TokenizerHelper
          .getTokenizerBuilder(penguinVersion)
          .enablePOSTagger
          .enableStopwordFilterWithNormalizer(normalizer)
          .setStopwordResourcePath("com/X/ml/feature/generator/stopwords_extended_{LANG}.txt")
          .enableStemmer
          .build
      }.toOption
  }
