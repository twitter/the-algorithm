...
import com.X.visibility.rules.ConversationControlRules._
import com.X.visibility.rules.FollowerRelations.AuthorMutesViewerRule
import com.X.visibility.rules.FollowerRelations.ProtectedViewerRule
import com.X.visibility.rules.PolicyLevelRuleParams.ruleParams
import com.X.visibility.rules.PublicInterestRules._
import com.X.visibility.rules.SafeSearchTweetRules._
import com.X.visibility.rules.SafeSearchUserRules.SafeSearchNsfwAvatarImageUserLabelRule
import com.X.visibility.rules.SafeSearchUserRules._
import com.X.visibility.rules.SpaceRules._
import com.X.visibility.rules.ToxicityReplyFilterRules.ToxicityReplyFilterDropNotificationRule
import com.X.visibility.rules.ToxicityReplyFilterRules.ToxicityReplyFilterRule
import com.X.visibility.rules.UnsafeSearchTweetRules._
import com.X.visibility.rules.UserUnavailableStateTombstoneRules._
...
abstract class VisibilityPolicy(
  ...
  val XArticleRules: Seq[Rule] = Nil,
  ...
  def forContentId(contentId: ContentId): Seq[Rule] =
    contentId match {
      ...
      case ContentId.XArticleId(_) => XArticleRules
  ...
  private[visibility] def allRules: Seq[Rule] =
    (tweetRules ++ userRules ++ cardRules ++ quotedTweetRules ++ dmRules ++ spaceRules ++ dmConversationRules ++ dmEventRules ++ XArticleRules ++ deletedTweetRules ++ mediaRules ++ communityRules)
}
...
case object XDelegateUserListPolicy
    extends VisibilityPolicy(
      userRules = Seq(
        ViewerBlocksAuthorRule,
...
case object XArticleComposePolicy
    extends VisibilityPolicy(
      XArticleRules = Seq(
        ViewerIsNotAuthorDropRule
      )
...
case object XArticleProfileTabPolicy
    extends VisibilityPolicy(
      XArticleRules = Seq(
        AuthorBlocksViewerDropRule
      )
...
case object XArticleReadPolicy
    extends VisibilityPolicy(
      XArticleRules = Seq(
        AuthorBlocksViewerDropRule,
      )
...
case class MixedVisibilityPolicy(
  ...
  additionalTweetRules: Seq[Rule])
    extends VisibilityPolicy(
      ...
      XArticleRules = originalPolicy.XArticleRules,
